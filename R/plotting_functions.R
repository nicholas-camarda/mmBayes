...
#' @export
create_bracket_dashboard_html <- function(bracket_year, decision_sheet, candidates, backtest = NULL, play_in_resolution = NULL, total_points_predictions = NULL) {
    ...
    paste0(
        ...
        ".legend-chip{display:inline-flex;align-items:center;gap:8px;padding:8px 10px;border-radius:999px;background:white;border:1px solid #d6d3d1;font-size:13px;}",
...
render_calibration_svg <- function(calibration_tbl) {
    ...
    paste0(
        ...
        points_html,
        "<text x='", margin_left, "' y='", height - 4, "' font-size='11' fill='#6b7280'>0%</text>",
        "<text x='", margin_left + plot_width, "' y='", height - 4, "' text-anchor='end' font-size='11' fill='#6b7280'>100%</text>",
        "<text x='10' y='16' font-size='11' fill='#6b7280'>Empirical</text>",
        "</svg>"
    )
}
...
#' @return A complete HTML document as a scalar character string.
#' @export
create_technical_dashboard_html <- function(bracket_year, decision_sheet, candidates, backtest = NULL, total_points_predictions = NULL, play_in_resolution = NULL) {
    top_decisions <- decision_sheet %>%
        dplyr::arrange(dplyr::desc(decision_score), round, region, matchup_number) %>%
        dplyr::mutate(
    ...
    paste0(
        ...
        "<div class='panel'><h2>Ranked Decision Board</h2><p class='panel-caption'>Top review spots sorted by decision score. Read left to right: matchup, favorite-side interval, then the recommended and alternate pick context.</p>",
        render_ranked_decision_svg(decision_sheet),
        "</div>",
        "<div class='two-column'>",
        "<div class='panel'><h2>Round-by-Region Risk Map</h2><p class='panel-caption'>Each cell summarizes the highest review pressure in that round-region slice.</p>",
        render_round_risk_heatmap_svg(decision_sheet),
        "</div>",
        "<div class='panel'><h2>Upset Opportunity Board</h2><p class='panel-caption'>Ordered by upset leverage so the best underdog pivots are obvious.</p>",
        render_upset_opportunity_svg(decision_sheet),
        "</div>",
        "</div>",
        "<div class='panel'><h2>Candidate Divergence</h2><p class='panel-caption'>Only the slots where the alternate bracket changes the path.</p>",
        ...
        if (!is.null(backtest) && !is.null(backtest$summary) && nrow(backtest$summary) > 0) {
            paste0(
                "<div class='two-column'><div>",
                render_html_table(calibration_summary),
                "</div><div>",
                render_calibration_svg(backtest$calibration %||% tibble::tibble()),
                "</div></div>"
            )
        } else {
