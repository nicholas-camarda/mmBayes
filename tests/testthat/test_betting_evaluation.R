...
test_that("optional closing-lines evaluation runs when local history exists", {
    closing_paths <- list.files(
        file.path("data", "odds_history"),
        pattern = "^closing_lines\\.csv$",
        recursive = TRUE,
        full.names = TRUE
    )
    if (length(closing_paths) == 0) {
        testthat::skip("No local closing lines found under data/odds_history/")
    }
    if (!file.exists(file.path("output", "model_quality", "latest_model_quality.rds"))) {
        testthat::skip("No output/model_quality/latest_model_quality.rds found; run the simulation pipeline first")
    }

    closing_lines <- dplyr::as_tibble(utils::read.csv(closing_paths[[1]], stringsAsFactors = FALSE))
    artifact <- readRDS(file.path("output", "model_quality", "latest_model_quality.rds"))
